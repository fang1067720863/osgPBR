...
public:
    ...
    using MaterialCache = std::map<int, osg::ref_ptr<osg::StateAttribute>>;
    mutable MaterialCache _materialCache;

    GLTFReaderV2() : _texCache(NULL)
    {
        //NOP
    }
        ...
        const osgDB::Options* readOptions) const
    {
        ...
        fs.FileExists = &GLTFReaderV2::FileExists;
        fs.ExpandFilePath = &GLTFReaderV2::ExpandFilePath;
        fs.ReadWholeFile = &GLTFReaderV2::ReadWholeFile;
        ...
        if (osgDB::containsServerAddress(location))
        {
            ...
            if (isBinary)
            {
                loader.LoadBinaryFromMemory(&model, &err, &warn, (const unsigned char*)mem.data(), mem.size(), baseDir, tinygltf::SectionCheck::REQUIRE_VERSION, &opt);
            }
            else
            {
                loader.LoadASCIIFromString(&model, &err, &warn, mem.data(), mem.size(), baseDir, tinygltf::SectionCheck::REQUIRE_VERSION, &opt);
            }
        }
        else
        {
    ...
    osg::Node* read(const std::string& location, const std::string& inputStream, const osgDB::Options* readOptions) const
    {
        ...
        if (magic == "glTF")
        {
            loader.LoadBinaryFromMemory(&model, &err, &warn, reinterpret_cast<const unsigned char*>(data->c_str()), data->size(), "", tinygltf::SectionCheck::REQUIRE_VERSION, &opt);
        }
        else
        {
            loader.LoadASCIIFromString(&model, &err, &warn, data->c_str(), data->size(), "", tinygltf::SectionCheck::REQUIRE_VERSION, &opt);
        }
    ...
    struct NodeBuilder
    {
        const GLTFReaderV2* reader;
        const tinygltf::Model& model;
        const Env& env;
        std::vector< osg::ref_ptr< osg::Array > > arrays;

        NodeBuilder(const GLTFReaderV2* reader_, const tinygltf::Model& model_, const Env& env_)
        ...
        osg::ref_ptr<osg::StateAttribute> makeMaterialFromModel(int materialIndex) const
        {
            auto& matCache = reader->_materialCache;
            auto matItr = matCache.find(materialIndex);
            if (matItr != matCache.end())
            {
            ...
            if (materialIndex >= 0 && materialIndex < model.materials.size())
            {
                ...
                if (material.alphaMode != "OPAQUE")
                {
                    ...
                    {
                        pbrMat->setAlphaMode(StandardPBRMaterial::AlphaMode::Mask);
                    }
                }
                ...
                for (tinygltf::ParameterMap::const_iterator paramItr = material.values.begin(); paramItr != material.values.end(); ++paramItr)
                {
                    if (paramItr->first == "baseColorFactor")
                    {
                        ...
                        pbrMat->setBaseColorFactor(osg::Vec4(color[0], color[1], color[2], color[3]));

                    }
                    if (paramItr->first == "metallicFactor")
                    {
                        double strength = paramItr->second.TextureStrength();
                        pbrMat->setAoStrength(strength);
                    }
                    if (paramItr->first == "roughnessFactor")
                    {
                        auto factor = paramItr->second.Factor();
                        pbrMat->setRoughnessFactor(factor);
                    }
                    if (paramItr->first == "metallicFactor")
                    {
                        auto factor = paramItr->second.Factor();
                        pbrMat->setMetallicFactor(factor);
                    }
                    else
                    {
                        // OE_DEBUG << "    " << paramItr->first << "=" << paramItr->second.string_value << std::endl;
                ...
                for (tinygltf::ParameterMap::const_iterator paramItr = material.values.begin(); paramItr != material.values.end(); ++paramItr)
                {
                    ...
                    if (paramItr->first == "normalTexture")
                    {
                        std::map< std::string, double>::const_iterator i = paramItr->second.json_double_value.find("index");
                        if (i != paramItr->second.json_double_value.end())
                        {
                    ...
                    if (paramItr->first == "metallicRoughnessTexture")
                    {
                        std::map< std::string, double>::const_iterator i = paramItr->second.json_double_value.find("index");
                        if (i != paramItr->second.json_double_value.end())
                        {
                    ...
                    if (paramItr->first == "occlusionTexture")
                    {
                        std::map< std::string, double>::const_iterator i = paramItr->second.json_double_value.find("index");
                        if (i != paramItr->second.json_double_value.end())
                        {
                    ...
                    if (paramItr->first == "emissiveTexture")
                    {
                        std::map< std::string, double>::const_iterator i = paramItr->second.json_double_value.find("index");
                        if (i != paramItr->second.json_double_value.end())
                        {
        ...
        osg::Group* makeMesh(const tinygltf::Mesh& mesh, bool prepInstancing) const
        {
            ...
            for (size_t i = 0; i < mesh.primitives.size(); i++) {
                ...
                osg::ref_ptr<osg::StateAttribute> pbrMat; 
               
                if (primitive.material >= 0 && primitive.material < model.materials.size())
                {
                    pbrMat = makeMaterialFromModel(primitive.material);
                }
...
#endif // OSGEARTH_GLTF_READER_V2_H
